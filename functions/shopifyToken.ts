...
Deno.serve(async (req) => {
  ...
  const client_secret = Deno.env.get('SHOPIFY_ADMIN_API_TOKEN');
  ...
  return Response.json({ access_token: data.access_token, expires_in: data.expires_in, scope: data.scope });
});