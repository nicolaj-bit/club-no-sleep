...
export default async function(req) {
  try {
    ...
    // Find accepted or pending FamilyInvite
    const invites = await base44.asServiceRole.entities.FamilyInvite.filter(
      { invitee_email: user.email },
      '-created_date',
      10
    );
    const invite = invites?.find(i => i.status === 'accepted' || i.status === 'pending');
    if (!invite) {
      return Response.json({ is_invited: false });
    }
    ...
    if (permissions.can_see_calendar) {
      inviterCalendarEvents = await base44.asServiceRole.entities.CalendarEvent.filter({ user_email: inviterEmail }, 'start_datetime', 60);
    }
    ...
    console.error('getInvitedData error:', error.message);
